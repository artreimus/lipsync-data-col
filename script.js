...
start_button.addEventListener('click', function () {
  ...
  media_recorder.addEventListener('stop', function () {
    video_local = URL.createObjectURL(
      new Blob(blobs_recorded, { type: 'video/mpg' })
    );
    download_link.href = video_local;

    let name_lastname = document.getElementById('name-lastname').value;
    let phrase = document.getElementById(counter).textContent;
...
window.onload = function fill_carousel() {
  for (let i = 0; i < phrases.length; i++) {
    ...
    document
      ...
      .insertBefore(elem, nav_container);
  }
};
...
function done() {
  document
    .getElementById('slideshow-container')
    .removeChild(document.getElementById(counter));
  let children = document
    .querySelector('#slideshow-container')
    .querySelectorAll('.mySlides1');
